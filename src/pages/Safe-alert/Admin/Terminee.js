...
export default function Terminee({alerts, statutColors, openModal}){
    return(
        <React.Fragment>
            <Card>
                <CardBody>
                    <h4 className="card-title mb-4">Terminee</h4>
                    <div className="table-responsive">
                    <table className="table table-nowrap align-middle mb-0">
                        <tbody>
                        {
                            (alerts .filter(alerte =>["terminee"].includes(alerte.statut))).map((alerte, index) => (
                                <tr 
                                    key={index}
