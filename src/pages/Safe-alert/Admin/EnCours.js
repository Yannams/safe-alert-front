...
export default function EnCours({alerts, statutColors, openModal}){
    return(
        <React.Fragment>
            <Card>
                <CardBody>
                    <h4 className="card-title mb-4">En cours</h4>
                    <div className="table-responsive">
                    <table className="table table-nowrap align-middle mb-0">
                        <tbody>
                        {
                            (alerts .filter(alerte =>["en cours"].includes(alerte.statut))).map((alerte, index) => (
                                <tr 
                                    key={index}
